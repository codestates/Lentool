...
const src = [
  ...
  ["https://cdn-icons.flaticon.com/png/512/5053/premium/5053011.png?token=exp=1652336855~hmac=da2be43a14ced71d15b1ffa3ff490b55", 'etc'],
]
  ...
  return (
    <div className="relative bg-white">
      <div className="md:flex space-x-10">
        ...
        <div className={`${isTag} absolute z-50 -ml-4 mt-3 transform px-2 w-screen max-w-sm sm:px-0 lg:ml-0 lg:left-1/2 lg:-translate-x-1/2`}>
              ...
              <div className='grid grid-cols-4 gap-4 justify-items-center'>
                {
                  src.map((el, index) => {
                    return (
                      <Link key={index} to={`/search?tag=${el[1]}`} onMouseDown={(e) => e.preventDefault()} onClick={() => searchByTag(el[1])}>
                        <img className="w-10 h-10 hover:shadow hover:border hover:rounded-lg" src={el[0]} alt={el[1]} />
                      </Link>
                    )
                  })
                }
